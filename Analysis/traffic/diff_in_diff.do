...
******************************** Export Table ********************************** 
	...
	* Export table (Latex) 
    ...
    station, month, day of week, and rush time fixed effects. Standard errors are clustered at the highway section level. Significance levels are indicated by asterisks: * p$<$0.05, ** p$<$0.01, *** p$<$0.001." ;
    #delimit cr

	loc table_path =  "$path_output/Table/table_1_DD_2.tex"
	if (${output_overleaf} == 1) {
		loc table_path =  "$path_overleaf/Tables/table_1_DD_2.tex"
	}
	
	esttab 	reg_vsr reg_sr reg_lr  ///
  	using 	"`table_path'", ///
            ...
            keep(1.post_treatment_2 1.group#1.post_treatment_2) ///
  			varlabels(1.post_treatment_2 "Post Treatment" 1.group#1.post_treatment_2 "I15 Provo to SLC $\times$ Post Treatment") ///
  			mgroups("Very Short Run" "Short Run" "Long Run", pattern(1 1 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///
  			prehead(`"\begin{table}[H]\centering"' `"\caption{@title}"' `"\begin{adjustbox}{width=${table_scale}\textwidth}"' `"\begin{tabular}{l*{@E}{c}}"' `"\toprule"') ///
  			title("Difference in Differences") mlabels(none) nonum posthead("`numbers'") ///
  			postfoot(`"\bottomrule"' `"\end{tabular}"' `"\end{adjustbox}"' `"\begin{minipage}[c][-3ex][t]{${table_scale}\textwidth}"'`"`note' "' `"\end{minipage}"' `"\end{table}"') ///
